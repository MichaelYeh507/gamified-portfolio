...
**Then the art pass** from the same pack's fences (three panels), barrels,
crates, lamps (two) and bucket through the retint tool — the recipe words
are all in place (`include` by node name, `targetHeight`, the pack's atlas
snap). Then the plaza in portrait (`KNOWN-ISSUES.md` 23) as a design call,
