...
**THE CODE TRACK PICKS UP HERE — THE SITE IS LIVE, THE ART PASS IS THREE
ROUNDS DEEP (7–9 Sep): the paddock corner, the paved plaza, the balloon and
the string lights between the boards stand; the crate stacks are cut; the
projects road leaves from under the car and rounds the name. Everything
since 7 Sep is staged, not committed — the messages are handed over.**

The 9 Sep session, in one line each (the *Now* entry above has the numbers):

- **The projects road** goes around the name instead of starting behind it:
  from under the car to the screen-left, round the name's left end (the
  right end is against the river — mapped), right under the tagline to the
  plaza's rim; the car spawns facing along it (`placeAtStart` reads the
  road; `areas.js` landing heading −0.76, guarded). `PROJECTS_ROAD` is the
  first route authored point by point; the `screen()` helper is the
  `[across, toward]` frame in `wayfindingPlan`.
- **The crate stacks cut** from the plaza ("remove the boxes between each
  project panel"); `crate` off the load list, prepped still.
- **The paddock's rails knockable** ("make the fence below edgeball project
  board knockable"): the flag, plus a hand's gap between panels so none is
  born touching (`panelStep`, `cornerGap`); nothing on the plaza is fixed
  now. Stood through 600 ticks, knocked flat by the car.

**Still open from his drive:** none named. **His look calls** and the rest
below stand.

---

**The previous handoff (7–8 Sep) is kept as written below.**

**THE CODE TRACK PICKS UP HERE — THE SITE IS LIVE, THE ART PASS IS TWO
ROUNDS DEEP (7 Sep, evening): the paddock corner stands on the plaza,
waiting for Michael's drive; the sundial was placed and cut the same
evening, and his brief for the plaza is "more beautiful and aesthetic
decorations".**

The 7 Sep evening session, in one line each (the *Now* entries above have
the measurements):

- **Round two placed:** the three rail panels as a paddock corner up-screen
  of the plaza's haystack (fixed bodies, the corridor fence's rule), loaded
  (`DRESSING_FILES`) and placed by the new pure plan
  `ProjectsArea.plazaDressing`, which also took over the crate stacks and
  the rim from round one, so the plaza's seventeen bodies are swept by
  `check-wayfinding`'s new seventh family (flat disc, road, disc edge,
  landing, lamps, posts, a separating-axis born-overlap test, the corner's
  geometry). Twelve suites green. Not committed — the messages are handed
  over.
- **The string lights (8 Sep).** Twenty amber bulbs on near-black cords
  between the boards' tops — one span per neighbouring pair along the arc,
  hooked at the title plates' corners, sagging 0.45, the lowest bulb 2.64
  over the paving; code-built (`plazaStringLights`, `PLAZA_LIGHTS`), the
  bulbs on the emissive band so the boards read as one lit gallery at night.
  They hung between the lamp posts for an hour first; his first look — "the
  car phases through the line" — moved them (the lamps are 2.7 over a 1.2
  car; the finials would have put the cord across the landed car's rear on
  screen). `Game.LAMP_PLACEMENTS` derives from `plazaLamps`. Seven guards
  per span plus the rows against the landed car. The agreed pair with the
  balloon is complete.
- **The balloon (8 Sep).** Michael's find, Styro's CC-BY balloon: prepped at
  5.5 (the sky blue assigned to the warm accent by rule, the burner on the
  emissive band), tethered as a bodiless visual behind the middle board off
  the axis, bobbing on a code-built rope, five new guards in the plaza
  family (column clear of every board's top, top inside the arrival frame).
  `balloonPack` byte-compared by `check-prep`. Staged, not committed.
- **The plaza floor, paved.** Michael's next note ("is the plaza just sitting
  on a non textured dome right now"): the floor was a flat dirt-coloured disc
  mesh from 20 Aug, a stand-in never replaced when the roads got their paving.
  The terrain now paints the disc into the paving channel with the road's
  feather (`plazaSlab` in `Terrain`); the disc mesh is gone. The plaza is a
  paved circle the road flows into.
- **The sundial, placed and cut.** It stood on the plaza's axis for an hour;
  Michael asked what its point was, and the answer was none (the sun's
  direction is fixed by design, so it could not even tell the time). It
  stays prepped in the recipe, unloaded. **His brief in the same breath:
  the plaza wants "more beautiful and aesthetic decorations"** — pieces
  with a look rather than props with a job. Searches for him: fountain,
  planters and flowerbeds, benches, banners, a statue, a well (CC0/CC-BY;
  the CC filter under Downloadable on Sketchfab; Kenney, Quaternius, Poly
  Pizza, KayKit). From the packs in hand the medieval flowers already stand
  as beds; nothing else in them is ornamental. **Agreed the same evening,
  the first two decorations:** string lights between the two lamp posts (a
  catenary of amber bulbs on the emissive band, code-built like the goal and
  the letters, so the disc is lit from above at night) and a tethered
  hot-air balloon on the rim (a found model, bobbing on its rope, a lit
  basket at night — his search: "hot air balloon low poly", Poly Pizza or
  Sketchfab with the CC filter; watch the envelope's triangles). A windmill
  on the ridge behind the boards was the third candidate, not taken yet.
- **Nothing new in Downloads.** The unused pieces of the packs in hand, no
  licence work needed, for round three: the medieval pack's anvil (the
  reference's plaza is ringed with an anvil, an oven and a grinder — "most
  similar to his"), three sacks, a ladder, four firewood stacks (the contact
  fire), a water bucket and an empty bucket; the human-props pack's second
  lantern (`Lamp_2`, 166 triangles) and three books. All measured in the
  session's node census; recipes are one line each.
- **The two look calls, measured, untouched — his to make:**
  1. *The lantern's lit pane at night.* The pane is 14 triangles over
     0.65–0.72 of the model's height (a 14 cm pane at the 2.0 recipe height)
     inside a 59-triangle dark frame (`#383838`, 0.64–0.77) and a
     135-triangle grey housing (`#636363`, 0.36–0.88). Levers, in order: add
     `'#383838': { color: 'amber', material: 'paletteEmissive' }` to the
     lantern's `colorOverrides` (the whole lantern box lights, the way the
     streetlight's glass does — the cage lines go, the lit volume triples);
     or raise `targetHeight` (2.4 makes a 17 cm pane, still a candle); the
     emissive intensity is shared by every night material, not a lever for
     one prop.
  2. *The channel-head puddle on the career road.* The contact-career road's
     centre line bottoms out at (−34.7, −1.5), 2 cm above the water surface
     (h −0.278 against −0.3), four units past the contact-career deck's far
     end toward the career post; the other two roads bottom out at 0.00 and
     −0.004. Leave it as the wet crossing it is, or move the route's bow in
     `wayfindingPlan` so the road clears the channel head (and re-run the
     bridge plan: a moved bow can change what the plan calls wet).
- **The plaza in portrait (`KNOWN-ISSUES.md` 23) — the design call, levers
  laid out.** Three boards need 20.34 across at their own screen row; the
  frame at that row is 28.21 at 16:9, 15.96 at 3:4, 13.73 at 9:16, 12.53 at
  9:19.5. (a) `ARC_STEP` 16°: to fit 9:16 the outer centres must come within
  9.9 of each other, a step of about 9.5°, and the ground gap between boards
  (2 · 30 · sin(step/2)) drops from 8.35 to 4.97 against 3.80-wide boards —
  1.2 of daylight, and the car (1.7 wide) no longer drives between them.
  (b) `ARC_RADIUS` 30: a smaller radius bends the wings toward the camera
  and closes the across-gap the same way without widening the row. (c)
  `ARC_AHEAD` 5.5: the frame is a trapezoid, 17.83 across at the bottom edge
  and 35.10 at the top, so boards further ahead sit in a wider row — but the
  top of the stack leaves the frame past about 10 ahead (the table in
  `ProjectsArea`), and at 9:16 the row is 20 wide only around 24 ahead. (d)
  A portrait-only layout moves the boards under a rotating phone and breaks
  the one-authoring rule. **Recommendation: accept — a portrait visitor
  drives along the arc.** The arrival frame shows the middle board whole with
  the wings visibly there, the greeting hop pulls the eye to each as you
  reach it, and the map nudge already says the world is bigger than the
  screen. If he takes it, close 23 as a decision rather than a defect.
- **The pitch's score and second goal — not built, a sketch:** `Pitch.js`
  has no goal detection at all. The shape: on a tick, the ball's centre
  inside the mouth box (`goal.x/z`, `heading`, `GOAL.width` × depth) scores
  once until the ball leaves it; the count as a flat road-paint number in
  front of the goal, the corridor's year-counter construction (ground text
  reads perfectly from the fixed camera; a board would not); the second goal
  mirrored through the pitch centre with its mouth facing the first, in
  `pitchPlan` so `check-pitch` holds both on dry flat ground with the ring
  clear; the reset prompt stays with the first goal.
- **A trap for the ritual:** `git checkout -- <file>` to undo a made-to-fail
  flip discards every uncommitted edit in the file. Copy the file before
  flipping and restore from the copy.

**What the next session does, in order:** Michael's drive (the paddock, the
paved plaza, the balloon, the string lights) → any further decoration he
finds (fountain, planters, benches, banners, a statue, a well) → his two
look calls → the rest of round three from the packs in hand (anvil first)
→ the portrait decision → the pitch's score and second goal → toasts.

---

**The previous handoff (6 to 7 Sep) is kept as written below; its item 1
landed for the fences (the sundial was placed and cut), items 2–4 carry
above.**

**THE CODE TRACK PICKS UP HERE — THE SITE IS LIVE, THE BRIDGES ARE IN,
THE ART PASS IS ONE ROUND AND ONE FIND DEEP (6 to 7 Sep); round two next.**

The 6 to 7 Sep session, in one line each (every item has a *Now* entry
above with the measurements):

- **Bridges** — artikora's pontoon (`humanPropsPack`, the new `footprint`
  recipe word for a non-uniform scale) over the two crossings where the
  whole road is wet, found by `bridgePlan` over the ford-only grid field;
  one fixed collider per deck; **no ford where a bridge stands** (his call
  the same evening: the river keeps its bed under the deck, `coverAt`);
  deck top flush with the banks at −0.03 (his second drive: "in the
  ground"). The third carved span, the channel-head skim on the career
  road, stays a ford by rule. Committed.
- **The art pass, round one** — lanterns at the deck ends and as a gate
  before the plaza (the pane on the emissive band via `colorOverrides`,
  a snap override keyed by sampled atlas colour), a keg and cube crate on
  the plaza rim, a camp at the contact fire; every placement a pure plan
  (`dressingPlan`, `contactPlan.camp`) swept by the suites. Rail fences
  and the sundial prepped, not loaded. Committed.
- **His first drive's notes** — the map nudge after the launch sheet
  (`FastTravel.nudge`, once, 12 s, hidden at the first open); rods as
  visuals, never bodies; the fishing spot out of the spawn's first drive.
  Committed.
- **The rowboat** (Muyaya Concept, CC-BY; a dock house on Sketchfab
  *Standard* was declined — that licence forbids the file redistribution
  this public repo does) — afloat as a visual, draught 0.03 so the water
  plane stays under the floor planks. **The whole fishing scene moved to
  the career crossing** ("the main screen is too cramped"), the boat
  moored along the channel clear of the deck, the rod planted at the
  waterline the plan walks to, pitched about its butt and leaning across
  the screen so the fixed camera shows it rising over the water. **The
  phone row**: the map and controls pills stack at the right edge under
  30rem on touch. Committed through the boat and the row; the rod's last
  two fixes are staged.
- **Suites**: still twelve; `check-wayfinding` grew the bridges (family
  5) and the dressing (family 6); `check-prep` byte-compares two more
  packs. Every new guard made to fail once.

**What the next session does, in order:**

1. **Round two of the art pass**: the three rail-fence panels (a paddock
   corner by the plaza haystack, or the bridge approaches) and the sundial
   (not the fast-travel landing) — prepped, waiting for a place — plus any
   CC0/CC-BY packs Michael finds (the search list he has: dock or pier,
   benches, well or windmill, market stall, tent, mailbox; Kenney,
   Quaternius, Poly Pizza, KayKit; the CC filter on Sketchfab). Same path:
   measure, recipe, credit row the minute it lands, a pure placement plan
   the suite sweeps, screenshots per round, his drive.
2. **Two look calls still his**: the lantern's lit pane at night (14
   triangles, reads as a candle; the lever is the recipe height or the
   emissive scale), and the channel-head puddle on the career road (leave
   it, or nudge the contact-career bow so the road clears the head).
3. **The plaza in portrait** (`KNOWN-ISSUES.md` 23) as a design call with
   the levers laid out; then the pitch's score and second goal; then
   toasts.
4. **A lead, not a task**: the water rendered flat navy in two MCP
   screenshots once and never again (`KNOWN-ISSUES.md`, *Open*). If he
   reports a dark river, that entry has what was on screen.

---

**The previous handoff (3 to 6 Sep) is kept as written below; its items
0–0d all landed.**
...
0b. **The payload lever**: `@dimforge/rapier3d` (non-compat) behind a wasm
   ...
   path (`KNOWN-ISSUES.md` 25). **Demoted 3 Sep, late**: after the pixel
...
~~**So the next session starts with bridges**~~ — **landed 6 Sep, evening**
(the *Now* entry has the measurements): the pontoon over the two crossings
where the whole road is wet, found by `bridgePlan` over the height field,
one fixed collider over each deck, the fords' physics untouched,
`check-wayfinding` at 36 more checks. **Awaiting Michael's drive** on the
dev build (`npx vite --port 5190`), with three things to judge:

- the look — a squat, wide pontoon (4.6 × 11–13, rails ~0.5) in the
  atlas's one dark-wood band; the height 1.4 and the deck level −0.10
  are one number each in `bridgePlan.BRIDGE` / the recipe;
- ~~**the trunk crossing reads as a bridge over a sandbar**~~ — **his call,
  the same evening: "the bridges should be over water, we might have to
  modify the terrain a bit."** Taken: where a bridge stands there is no
  ford (`bridgePlan.coverAt` in `Terrain.carveAt`; the decks are planned
  over the ford-only field first). The bed under the trunk deck is −1.05,
  under the career deck −0.87; both ends still buried within 4 cm on the
  centreline and both edges; the shelf fades beside a deck at 0.18 per
  unit against the 0.273 bank gradient. A car leaving a deck sideways now
  drops into real water — accepted with the call;
- **the third "ford" carries no bridge, on purpose**: the career road
  skims the head of a southern channel with one edge in 0.02 of water and
  the other on the grass. A level deck there beaches on the lawn. Options
  if he wants it gone: leave it (a puddle at the road's edge), or nudge
  the contact-career bow (−3.5) so the road clears the channel head — a
  route change the whole suite would re-sweep.

**The art pass, round one, landed 6 Sep late** (the *Now* entry): the
pack's lantern post (emissive pane via the new `colorOverrides` recipe
word), keg, cube crate, bucket and rods, placed as swept plans — lanterns
at the deck ends and as a gate before the plaza, the fishing spot by the
trunk bridge, the camp at the contact fire, a keg and crate on the plaza
rim. **Awaiting Michael's drive.** Round two, on his call: the three rail
fences (a paddock corner by the plaza haystack, or the bridge approaches)
and the sundial (somewhere that is not the fast-travel landing) — both
prepped, not loaded; and any further packs he finds (CC0/CC-BY, the same
stylised low-poly look; the holes worth filling are a rowboat or dock for
the pontoons, benches, a well or windmill, a market stall, a tent for the
camp). Then the plaza in portrait (`KNOWN-ISSUES.md` 23) as a design call,
then the pitch's score and second goal.
...
2. **The reference's verdict on the session's look calls, judged by driving.** Still
   ...
   letters at 0.6, plaza titles blank until greeted (the crate stacks were
   judged 9 Sep: cut), leaf density at 256, the wind lines' cadence, rain at ~20 % of the
